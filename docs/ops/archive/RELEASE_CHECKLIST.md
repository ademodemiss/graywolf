...
- [x] `.env.example` provider şablonu ve secret rehberi
- [x] README kullanıcı kurulum adımları
- [x] `python -m py_compile tools/provider_setup_wizard.py tools/provider_validation.py`
- [x] Wizard dry-run komutları `logs/terminal.log` içinden doğrulandı
- [x] `tools/mail_tool.py --action check-smtp --provider <provider>` ile validation raporu alındı
